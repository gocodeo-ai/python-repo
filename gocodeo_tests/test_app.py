...
# happy_path - test_add_item - Test that item is added to the cart
def test_add_item(client, mock_cart):
...
# happy_path - test_remove_item - Test that item is removed from the cart
def test_remove_item(client, mock_cart):
...
# happy_path - test_update_item_quantity - Test that item quantity is updated in the cart
def test_update_item_quantity(client, mock_cart):
...
# happy_path - test_calculate_total_price - Test that total price is calculated for the cart
...
def test_apply_discount_to_cart(client, mock_cart, mock_discount):
    ...
    assert response.status_code == 200
    assert response.get_json() == {'discounted_total': 50.0}
...
# edge_case - test_remove_nonexistent_item - Test removing an item not present in the cart
...
# edge_case - test_apply_discount_insufficient_amount - Test applying discount with insufficient purchase amount
def test_apply_discount_insufficient_amount(client, mock_cart, mock_discount):
