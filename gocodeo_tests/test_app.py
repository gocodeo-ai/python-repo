...
def mock_get_all_items_from_cart():
    with patch('shopping_cart.utils.get_all_items_from_cart') as MockGetAllItems:
        MockGetAllItems.return_value = [{'item_id': 1, 'quantity': 5, 'price': 10.0, 'name': 'Test Item', 'category': 'Test Category'}]
...
# happy path - get_cart_items - Test that all items in the cart are retrieved successfully
def test_get_cart_items_success(client):
    response = client.get('/get_cart_items')
    assert response.status_code == 200
    assert response.json == {
        'items': [{'item_id': 1, 'quantity': 5, 'price': 10.0, 'name': 'Test Item', 'category': 'Test Category'}]
    }


# happy path - calculate_total_price - Test that total price of the cart is calculated successfully
...
# happy path - apply_discount_to_cart - Test that discount is applied to the cart successfully
...
# happy path - apply_promotions_to_cart - Test that promotions are applied to the cart successfully
...
def test_update_item_quantity_to_zero(client):
    response = client.post('/update_item_quantity', json={
        'item_id': 1,
        'new_quantity': 0
    })
...
# edge case - apply_discount_to_cart - Test applying discount with a rate higher than 1
def test_apply_discount_high_rate(client, mock_discount):
    mock_discount.apply_discount.side_effect = Exception('Invalid discount rate')
    response = client.post('/apply_discount', json={
        'discount_rate': 1.5,
        'min_purchase_amount': 0
    })
...
def test_apply_promotions_no_applicable(client, mock_apply_promotions):
    mock_apply_promotions.side_effect = Exception('No applicable promotions')
    response = client.post('/apply_promotions')
    assert response.status_code == 200
